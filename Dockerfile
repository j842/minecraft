...
RUN groupadd -g 22922 drgroup \
    ...
    && mkdir /minecraft \
    && cd /minecraft \
    && wget -q https://s3.amazonaws.com/Minecraft.Download/versions/1.9.4/minecraft_server.1.9.4.jar \
...
RUN chmod a-w -R /drunner && chmod a+r /minecraft*  \
    && echo "#!/bin/bash\ncd /minecraft/data\necho "eula=true" > eula.txt\njava -Xms1G -Xmx1G -jar /minecraft/minecraft_server.1.9.4.jar nogui" >> /usr/local/bin/runminecraft.sh \
